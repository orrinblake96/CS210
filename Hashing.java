...
//create the hash table
    ...
    public static void main (String[] args) throws IOException {
      File testFile = new File("/Users/orrinblake/Documents/dictionary.txt");     //this is where the file to be sorted is loaded from
...
//the whole thing is added together and then moduloed to create a hash table index
    	 ...
    	 int hash = 0; //set hash to 0
         int count = 0; //set count to 0, count used for succsessive powers
         for(int i=0;i<word.length();i++) { //loop through each letter in the word
        	char c = word.charAt(i); //get each letter from word
           int j = (int)c; //convert word to its int equivalent
           hash += j * modPow(258, count, size); //modulo the large powers to keep int in range
           count++; //next power
         }

         hash = hash % size; //get unique position in array
         return hash;
...
//for example, you could just add up all of the letters in the word
         ...
         for(int i=0;i<word.length();i++) { //loop through each letter in the word
        	char c = word.charAt(i); //get each letter from word
           j += ((int)c); //number will be different because this time were adding j back to it each time also
         }
         return j % size;//get pos within the array
     }
